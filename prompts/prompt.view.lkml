...
examples_cte AS (
  ...
  FROM ${examples.SQL_TABLE_NAME} WHERE agent_name = {{agent._parameter_value | replace: "\'", ""}}
),
...
extra_context_cte AS (
  ...
  FROM ${extra_context.SQL_TABLE_NAME} WHERE agent_name = {{agent._parameter_value | replace: "\'", ""}}
),
